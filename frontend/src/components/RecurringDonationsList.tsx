...
import { useGetRecurringDonations, useCancelRecurringDonation, useGetAllCampaigns } from '../hooks/useQueries';
...
export default function RecurringDonationsList() {
  ...
  const { mutateAsync: cancelRecurring, isPending: isCancelling } = useCancelRecurringDonation();

  const getCampaignTitle = (campaignId: bigint) => {
    return campaigns?.find((c) => c.id === campaignId)?.title || `Campaign #${campaignId}`;
  };
      ...
      <CardContent>
        {active.length === 0 ? (
          ...
          <div className="space-y-3">
            {active.slice(0, 5).map((donation, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 bg-accent/40 rounded-xl">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-foreground truncate">
                    {getCampaignTitle(donation.campaignId)}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {formatCurrency(Number(donation.amount) / 100)} / month · Since {formatDate(donation.timestamp)}
                  </p>
                </div>
                <Button
                  variant="ghost"
