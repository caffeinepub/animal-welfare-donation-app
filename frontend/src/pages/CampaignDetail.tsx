...
import { useGetCampaign, useGetAllCampaigns, useGetDonationHistory } from '../hooks/useQueries';
...
export default function CampaignDetail() {
  ...
  const { data: allDonations, isLoading: donationsLoading } = useGetDonationHistory();
  ...
  const handleDonationSuccess = (referenceId: string, amount: number, isRecurring: boolean) => {
  ...
  const donorCount = new Set(campaignDonations.map((d) => d.donor.toString())).size;
  ...
  if (!campaign) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        ...
        <h2 className="font-serif font-bold text-xl text-foreground">Campaign not found</h2>
        <p className="text-muted-foreground mt-1 mb-4">This campaign may have been removed.</p>
      ...
      <div className="relative rounded-2xl overflow-hidden mb-6 bg-accent h-56 md:h-80">
        {campaign.imageUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/10 to-secondary/20">
            <span className="text-8xl">{getAnimalEmoji(campaign.animalType)}</span>
          </div>
        )}
        <div className="absolute top-4 left-4 flex gap-2">
          ...
          <Badge variant="outline" className="bg-card/90 rounded-full backdrop-blur-sm font-semibold">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2 text-sm text-muted-foreground">
              ...
              <span>
                ...
                {campaign.endDate && campaign.endDate !== BigInt(0) && ` – ${formatDate(campaign.endDate)}`}
              </span>
            </div>
          </div>
